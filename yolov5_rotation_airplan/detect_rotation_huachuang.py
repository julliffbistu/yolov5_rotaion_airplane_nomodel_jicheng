...
def detect(weights='yolov5s.pt',  # model.pt path(s)
    ...
    for i,imgpath in enumerate(imglist):
        ...
        H,W,C=ori_img.shape
        IMG = np.zeros([int(chongfu*int(H/chongfu )+split_size), int(chongfu*int(W/chongfu)+split_size), 3], np.uint8)
        IMG[0:ori_img.shape[0],0:ori_img.shape[1]] = ori_img.copy()
        #print(IMG.shape)
        for x in range(0,IMG.shape[0]-split_size,chongfu):
            for y in range(0,IMG.shape[1]-split_size,chongfu):
                img = IMG[x:split_size+x,y:split_size+y].copy()
                #img=ori_img.copy()
